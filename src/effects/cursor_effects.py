...
class Particle:
    def __init__(self, start_x, start_y, speed, mode, shape, size: list[int], colour: tuple[int, int, int], prange: tuple[float, float], acceleration=False, glow=True):
        self.x = start_x
        self.y = start_y
        self.speed = speed
        self.mode = mode
    ...
    def draw(self, screen):
        ...
        match self.shape:
            ...
            case "circle":
                pygame.draw.circle(screen, self.colour, (self.x, self.y), tuple(self.size)[0])
        ...
        # screen.blit(circle_surf(tuple(self.size)[0], (20, 20, 20)),
        #             (self.x - tuple(self.size)[0], self.y - tuple(self.size)[1]),
...
class Cursor:
    ...
    def create_new_particle(self, mx, my):
        ...
        if self.count >= self.generation_rate:
            ...
            self.particles.append(Particle(mx, my, 3, "classic", "circle", [size, size], self.colour, (-0.7, 0.7)))
            self.count = 0
